...
int process_image_file(char* szFilename, char* szFilename_FAT1, char* szFilename_FAT2, int nPatchFat)
{
   FILE*               pFile = 0;
   int                 nStatus;
   ...
   // Read File System parameters, FAT1, FAT2.
   ...
   // Dump FAT table to binary file.
   nReturnValue = dump_fat_tables(
      szFilename_FAT1,
      szFilename_FAT2,
      pFAT1_Buffer,
      pFAT2_Buffer,
...
int dump_fat_tables(
   char* szFilename_FAT1,
   char* szFilename_FAT2,
   uint32_t* pFAT1_Buffer,
   uint32_t* pFAT2_Buffer,
   int32_t lFileAllocationTableSize)
{
   int nReturnValue = 0;
   int nStatus = 0;
   int32_t lBytesWritten = 0;
   int32_t lBytesToWrite = lFileAllocationTableSize;
   FILE* pOutputFile = 0;

   // Write out FAT1 to output file.
   pOutputFile = fopen(szFilename_FAT1, "wb");
   lBytesWritten = fwrite(pFAT1_Buffer, 1, lBytesToWrite, pOutputFile);
   nStatus = fclose(pOutputFile);

   // Write out FAT2 to output file.
   pOutputFile = fopen(szFilename_FAT2, "wb");
   lBytesWritten = fwrite(pFAT2_Buffer, 1, lBytesToWrite, pOutputFile);
   nStatus = fclose(pOutputFile);
...
#ifdef TIFDAT
   const unsigned char ulFilenameTIF_DAT [8] =
   {'T', 'I', 'F', ' ', ' ', ' ', ' ', ' '};

   const unsigned char ulExtensionTIF_DAT [3] =
   {'D', 'A', 'T' };

   /* Display 16 bytes of TIF.DAT */
   if ((0 == memcmp(pDirEntry->dosFilename, ulFilenameTIF_DAT, sizeof(ulFilenameTIF_DAT))) &&
      (0 == memcmp(pDirEntry->dosFilename, ulFilenameTIF_DAT, sizeof(ulFilenameTIF_DAT))))
   {
   }
#endif
